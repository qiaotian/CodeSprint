...
public:
    int maxCoins(vector<int> &nums) {
        ...
        // n is the length of the new array, which is generated by step1 and step2
        int n = 1;
        // 1. delete all zeros in nums
        for (int x : nums) if (x > 0) _nums[n++] = x;
        // 2. add 1 to head and tail of nums
        _nums[0] = _nums[n++] = 1;

        // The dp[i][j] means the max coins could get by bursting all ballons between ith and jth elements,
        // not including the ith and jth.
        // Therefore, the leftmost i is zero, and the rightmost j is n-1.
        int dp[n][n] = {};
        for (int k = 2; k < n; ++k) {
